...
module PackageManager
  class Base
    ...
    private_class_method def self.download_async(names)
      names.each_slice(20).each_with_index do |group, index|
        group.each { |name| PackageManagerDownloadWorker.perform_in(index.minutes, self.name, name, nil, "download_async") }
      end
    end
  end
end
